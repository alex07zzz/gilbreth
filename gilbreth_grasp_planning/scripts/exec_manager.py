...
#ARM_GROUP = ROBOT.get_group(ARM_GROUP_NAME)
RAIL_GROUP = ROBOT.get_group(RAIL_GROUP_NAME)
RAIL_GROUP.set_planning_time(1.0)
RAIL_GROUP.MoveGroupCommander.allow_replanning = True
...
class WaitingforMoveGroupState(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes = ['succeeded', 'failed'])
    def execute(self, wait_time = 10.0):
...
class WaitingState(smach.State):
    def __init__(self):
        smach.State.__init__(self, outcomes = ['new msg', ''])
class HomeState(smach.State):
    ...
    def execute(self):
        ...
        RAIL_GROUP.set_start_state_to_current_state()
        RAIL_GROUP.set_pose_target(self.waiting_pose)
...
class RobotExecution:
    def __init__(self):
        ...
        #self.rail_group = moveit_commander.MoveGroupCommander(RAIL_GROUP_NAME)
