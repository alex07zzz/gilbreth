...
class RobotExecution:
    def __init__(self):
        ...
        ## define an collision object
        self.collision_object = CollisionObject()
        self.collision_object.header.frame_id = self.rail_group.get_planning_frame()
        self.collision_object.id = "box1"
        primitive = SolidPrimitive()
        primitive.type = primitive.BOX
        primitive.dimensions.append(0.3)
        primitive.dimensions.append(0.3)
    ...
    def execute_robot(self):
      ...
      with ScopeExit(self) as sc:       
        ...
        if self.EXECUTE:
                ...
                if pick_traj:
                    ...
                    if (current_time + pick_dur > pick_deadline):
                        ...
                        if self.gripper_attached(pick_deadline):
                            ...
                            ## attach object
                            box_pose = Pose() 
                            box_pose = copy.deepcopy(target_tool_pose.pick_pose.pose)
                            self.collision_object.primitives.append(primitive)
                            self.collision_object.primitive_pose.append(box_pose)
                            self.collision_object.operation = collision_object.ADD
                            rospy.loginfo("Add an object into the world")
                            collision_objs = []
                            collision_objs.append(collision_object)
                            self.scene.addCollisionobjects(collision_objs)
                            self.group.attachObject(collision_object.id)
                            ...
                            if retreat_traj:
                                ...
                                if place_traj:
                                    ...
                                    try:
                                        ...
                                        rospy.loginfo("Placing duration is : %f" % place_dur.to_sec())
                                        rospy.sleep(retreat_dur.to_sec())
                                    except:
            ...
            self.group.detachObject(collision_object.id)
            self.scene.removeCollisionObjects(collision_object.id)
            self.tool_poses_list.pop(0)
            self.disable_gripper()
