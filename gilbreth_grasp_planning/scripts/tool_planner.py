...
class ToolPlanner():
    ...
    def compute_tool_poses(self):
        try: 
            ...
            rospy.loginfo("Current object is %s",name)
           ...
           #compute pick_approach pose (%lift meter above the pick up position)
            ...
            tool_msg.pick_approach.header.stamp = self.obj_data.detection_time + rospy.Duration(self.property.conveyor_time + self.property.approach_time)   
            tool_msg.pick_pose.header.frame_id = 'world_frame'
            tool_msg.pick_pose.header.stamp = self.obj_data.detection_time + rospy.Duration(self.property.conveyor_time + self.property.pick_time)
            tool_msg.pick_retreat.header.frame_id = 'world_frame'
            tool_msg.pick_retreat.header.stamp = self.obj_data.detection_time + rospy.Duration(self.property.conveyor_time + self.property.retreat_time)
            tool_msg.place_pose.header.frame_id = 'world_frame'
            tool_msg.place_pose.header.stamp = self.obj_data.detection_time + rospy.Duration(self.property.conveyor_time + self.property.place_time)
